    ...
    for meth in methods:
        ...
        # directed methods
            ...
            if meth in ['MCC']:
                ...
                aux = CONN(TE, data_uni, aux_dir, max_workers)
                print('TE complete')
            # comparing real data to surrogates
            aux2 = np.zeros((aux.shape[0], aux.shape[1], aux.shape[2]))
            for epc in range(aux.shape[2]):
...
def CONN(func, data, aux_dir, max_workers):
    ...
    seed = np.random.randint(1, 2147483647, (nepoch, nseg, nsurr))

    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        results = [executor.submit(func, data[epc][:, :, seg, sur], seed[epc, seg, sur], aux_dir, [epc, seg, sur]) for
                   epc, seg, sur in itertools.product(range(nepoch), range(nseg), range(nsurr))]
...
def MI5_EXEC(data, seed, aux_dir, info):
    ...
    savemat(os.path.join(aux_dir, 'aux_' + str(info[0]) + '_' + str(info[1]) + '_' + str(info[2]) + '.mat'),
            {'out': out})
...
def MIhigherdim(d1, d2):
    ...
    r = np.random.randint(2147483647)
...
def TE(data, *args):
    ...
    for ch1 in range(n_sens):
        for ch2 in range( n_sens):
            out[ch1, ch2] =eng.transfer_entropy(matlab.double(data[:, ch1].tolist()), matlab.double(data[:, ch2].tolist()))
...
def create_network(nets, TH, ALPHA=0.05):
    ...
    for ch1 in range(nets.shape[0]):
        for ch2 in range(nets.shape[1]):
...
def correct_network(network, h):
    ...
    # Correct the network
    count = 0
    for ch1 in range(network.shape[0] ):
        for ch2 in range(network.shape[1]):
